...
use oas3::{
    OpenApiV3Spec,
    spec::{
        ObjectOrReference, Operation, Parameter, ParameterIn, Response, Schema, SchemaType,
        SchemaTypeSet,
    },
};
use tracing::info;

use super::utils::{CollectWrites, camel_to_kebab, category_label, op_category, path_to_slug};
        ...
        Schema::Object(oor) => match oor.as_ref() {
            ObjectOrReference::Object(obj) => {
                ...
                let mut base = match ty {
                    None => "any".to_string(),
                    Some(SchemaType::Integer) => obj
                        .format
                        .as_deref()
                        .map(|f| format!("integer ({f})"))
                        .unwrap_or_else(|| "integer".to_string()),
                    Some(SchemaType::Number) => obj
                        .format
                        .as_deref()
                        .map(|f| format!("number ({f})"))
                        .unwrap_or_else(|| "number".to_string()),
                    Some(SchemaType::String) => obj
                        .format
                        .as_deref()
